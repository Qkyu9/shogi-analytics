...
- **同義の別名タグは使わない**（下記の正本表記に統一）
- 該当するものを1〜3個。複数可
- 候補例（正本表記）:
  - 序盤: **序盤の駒組み** / 型の選択・準備不足
  - 中盤（攻守の選択）: 中盤の読み（攻めるか受けるか）
  - 中盤（攻め）: 攻めの手順選択 / 攻めの駒組み / 玉頭・玉攻めの手筋 / 攻め駒の責め方 / 勝ち切りの手筋不足 / 端攻めの手順選択
  - 中盤（守り）: 守りの手筋選択 / 逆手への対応不足 / 想定外の手への対応不足
  - 終盤: 寄せの読み漏れ / **詰み逃し・逆転の見落とし** / **入玉対策**（入玉の場面ならこちらを優先）
  - その他: 時間切れ / 話の内容に合わせた短い名詞句（「ミス」禁止）
- 使わない別名（自動的に正本へ統合される）:
  - 序盤の手筋・序盤の手筋選択・駒組みの工夫 → 序盤の駒組み
  - 詰めの力不足・詰みの力不足 → 詰み逃し・逆転の見落とし
  - ※ 入玉対策と詰み逃しは上下関係があり、両方付与してよい
