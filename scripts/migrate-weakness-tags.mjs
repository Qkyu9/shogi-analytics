/**
 * 既存 game_records の敗因タグ同義語を正本表記に更新する。
 * 正本は app/lib/weakness-tags.ts の TAG_SYNONYM_MAP と同期すること。
 * 使い方: node scripts/migrate-weakness-tags.mjs
 */
...
const TAG_SYNONYM_MAP = {
  ...
  駒組みの工夫: "序盤の駒組み",
  駒組みの工夫不足: "序盤の駒組み",
  "詰め逃し・逆転の見落とし": "詰み逃し・逆転の見落とし",
  "積み逃し・逆転の見落とし": "詰み逃し・逆転の見落とし",
  詰めの力不足: "詰み逃し・逆転の見落とし",
  詰みの力不足: "詰み逃し・逆転の見落とし",
  詰め力不足: "詰み逃し・逆転の見落とし",
  中盤の勝ち切り不足: "勝ち切りの手筋不足",
...
function normalizeTag(tag) {
  ...
  if (!t) return "";
  if (TAG_SYNONYM_MAP[t]) return TAG_SYNONYM_MAP[t];
  if (t.endsWith("ミス")) {
